object Lab5 extends jsy.util.JsyApplication {
  ...
  // Will return type A, but will have input/output state of 
  /*
  ...
  //returns mode definition of paramater?
  
  def typeInfer(env: Map[String,(Mutability,Typ)], e: Expr): Typ = {
	    //was not a tuple of mut and typ, in lab 4
		  ...
		  case Function(p, paramse, tann, e1) => {
		        ...
		        val env1 = (p, tann) match {
		          case (Some(f), Some(rt)) =>
		            ...
		            env + (f -> (MConst, tprime)) //extending environment with a mutable tprime
		          case (None, _) => env
	      ...
	      case Assign(e1, e2) => 
		       ...
		       e1 match {
		         case Var(x) => env.get(x) match {
		             case Some((varX, typeOfe1)) if (typeOfe1 == typeOfe2) => typeOfe1
		             case _ => err(typeOfe2, e2)
		         } 
